...
from django.http import HttpResponse
import pdfkit
from .models import Profile, Skill, Referee, Academic
...
def generate_resume(request, id):
    ...
    user_profile = Profile.objects.filter(cv_id=id).first()  # Assuming cv_id is unique
    user_skill = Skill.objects.filter(cv_id=id).values()
    user_referee = Referee.objects.filter(cv_id=id).values()
    user_education = Academic.objects.filter(cv_id=id).values()
    ...
    html_template = 'core/pdf_template.html'
    rendered_html = render(request, html_template, context).content.decode('utf-8')
    ...
    # If PDF generation was successful, return the PDF for download
    ...
    return HttpResponse('PDF generation failed: {}'.format(pisa_status.err))
